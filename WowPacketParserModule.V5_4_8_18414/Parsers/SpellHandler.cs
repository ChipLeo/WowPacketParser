...
namespace WowPacketParserModule.V5_4_8_18414.Parsers
{
    public static class SpellHandler
    {
        ...
        public static void HandleCancelCast(Packet packet)
        {
            ...
            if (hasSpellID) // 16
                packet.ReadUInt32<SpellId>("Spell ID"); // 16
        ...
        {
            packet.ReadUInt32<SpellId>("Spell ID"); // 36
        ...
        public static void HandleSpellChannelStart(Packet packet)
        {
            ...
            packet.ReadUInt32<SpellId>("Spell ID"); // 144
        ...
        public static void HandleCooldownEvent(Packet packet)
        {
            ...
            packet.ReadInt32<SpellId>("Spell ID"); // 24
        ...
        public static void HandleMissileCancel(Packet packet)
        {
            ...
            packet.ReadInt32<SpellId>("Spell ID"); // 64
        ...
        public static void HandleModifyCooldown(Packet packet)
        {
            ...
            packet.ReadUInt32<SpellId>("Spell ID"); // 28
        ...
        public static void HandleProcResist(Packet packet)
        {
            ...
            if (unk48)
                packet.ReadSingle("unk44"); // 44
            packet.ParseBitStream(guid, 5, 7);
            packet.ParseBitStream(guid2, 4, 0);
            packet.ReadInt32<SpellId>("Spell ID"); // 160
            packet.ParseBitStream(guid2, 2);
            packet.ParseBitStream(guid, 0);
            packet.ParseBitStream(guid2, 3);
        ...
        public static void HandleResumeCastBar(Packet packet)
        {
            ...
            packet.ReadInt32<SpellId>("Spell ID"); // 60
        ...
        public static void HandleSpellCooldown(Packet packet)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                packet.ReadInt32<SpellId>("Spell ID"); // 80
        ...
        public static void HandleSpellFailedOther(Packet packet)
        {
            ...
            packet.ReadByteE<SpellCastFailureReason>("Reason"); // 29
        ...
        public static void HandleSpellGo(Packet packet)
        {
            ...
            var spellId = packet.ReadUInt32<SpellId>("Spell ID"); // 68
        ...
        public static void HandleSpellStart(Packet packet)
        {
            ...
            packet.ReadBit("!unk30"); // 30

            guid2[4] = packet.ReadBit(); // 28
            guid[2] = packet.ReadBit(); // 18

            var runeCooldownPassedCount = packet.ReadBits("runeCooldownPassedCount", 3);
            ...
            var hasSourceLocation = packet.ReadBit("hasSourceLocation");
            var predictedPowerCount = packet.ReadBits("predictedPowerCount", 21);

            var guid120 = packet.StartBitStream(3, 0, 1, 7, 2, 6, 4, 5);

            var hasElevation = !packet.ReadBit("!hasElevation");

            var hasTargetString = !packet.ReadBit("!hasTargetString"); // 0x80000000 48

            var hasAmmoInventoryType = !packet.ReadBit("!hasAmmoInventoryType");
            var hasDestLocation = packet.ReadBit("hasDestLocation");
            ...
            var hasAmmoDisplayId = !packet.ReadBit("!hasAmmoDisplayId");

            var guid128 = new byte[8];
            if (hasSourceLocation)
                guid128 = packet.StartBitStream(4, 3, 5, 1, 7, 0, 6, 2);

            packet.ReadBit("!unk104");

            guid[6] = packet.ReadBit();

            var guid416 = packet.StartBitStream(2, 1, 7, 6, 0, 5, 3, 4);

            var hasTargetMask = !packet.ReadBit("!hasTargetMask");
            if (hasTargetMask)
                packet.ReadBits("TargetMask", 20);

            guid[1] = packet.ReadBit();

            var hasPredictedHeal = !packet.ReadBit("!hasPredictedHeal");
            var unk336 = !packet.ReadBit("!unk336");
            var hasCastSchoolImmunities = !packet.ReadBit("!hasCastSchoolImmunities");

            guid2[5] = packet.ReadBit();

            packet.ReadBit("!unk28");
            ...
            var targetStrLen = 0u;
            if (hasTargetString)
                targetStrLen = packet.ReadBits("targetStrLen", 7);
            ...
            var hasCastImmunities = !packet.ReadBit("!hasCastImmunities");

            guid2[1] = packet.ReadBit();

            var hasVisualChain = packet.ReadBit("hasVisualChain");
            guid[7] = packet.ReadBit();
            var hasPredictedType = !packet.ReadBit("!hasPredictedType");
            guid2[0] = packet.ReadBit();
            ...
            if (hasSourceLocation)
            {
                ...
                packet.WriteGuid("guid128", guid128);
            }
            ...
            {
                packet.ReadInt32("unk324", i);
                packet.ReadInt32("unk328", i);
            }

            if (unk336)
                packet.ReadByte("unk336");

            CastFlag flags = packet.ReadInt32E<CastFlag>("Cast Flags");
            ...
            if (targetStrLen > 0)
                packet.ReadWoWString("TargetStr", targetStrLen);
            ...
            {
                packet.ReadByte("runeCooldownPassed", i);
            }
        ...
        public static void HandleSpellInstaKillLog(Packet packet)
        {
            ...
            packet.ParseBitStream(guid, 1);
            packet.ReadInt32<SpellId>("Spell ID"); // 32
            packet.ParseBitStream(guid2, 3);
            packet.ParseBitStream(guid, 2);
        ...
        public static void HandleSpellInterruptLog(Packet packet)
        {
            ...
            packet.ParseBitStream(guid, 0);
            packet.ParseBitStream(guid2, 2);
            packet.ReadInt32<SpellId>("Interrupt Spell ID"); // 128
            packet.ParseBitStream(guid2, 1);
            packet.ParseBitStream(guid, 2);
            packet.ParseBitStream(guid2, 3);
            packet.ReadInt32<SpellId>("Interrupted Spell ID"); // 144
            packet.ParseBitStream(guid, 4);
            packet.ParseBitStream(guid2, 4);
        ...
        public static void HandleSupercededSpell(Packet packet)
        {
            ...
            for (var i = 0; i < count32; i++)
                packet.ReadUInt32<SpellId>("New Spell ID", i); // 36
            for (var i = 0; i < count16; i++)
                packet.ReadUInt32<SpellId>("Old Spell ID", i); // 20
        }
        ...
        public static void HandleTotemCreated(Packet packet)
        {
            ...
            packet.ReadInt32<SpellId>("Spell ID"); // 96
        ...
        [Parser(Opcode.SMSG_UNK_10D8)]
        public static void HandleUnk10D8(Packet packet)
        {
            packet.ReadBit("!unk24"); // 24
            var guid24 = packet.StartBitStream(0, 1, 4, 7, 3, 6, 2, 5);
            var unk32 = !packet.ReadBit("!unks32"); // 32
            var unk40 = !packet.ReadBit("!unks40"); // 40
            var unk48 = !packet.ReadBit("!unk48"); // 48
            var unk44 = !packet.ReadBit("!unk44"); // 44
            var unk36 = !packet.ReadBit("!unk36"); // 36
            packet.ReadBit("!unk16"); // 16
            var guid16 = packet.StartBitStream(7, 4, 1, 5, 3, 0, 2, 6);

            packet.ParseBitStream(guid16, 7, 6, 4, 1, 0, 5, 2, 3);
            if (unk44)
                packet.ReadInt32("unk44"); // 44
            if (unk40)
                packet.ReadSingle("unk40"); // 40
            packet.ParseBitStream(guid24, 7, 4, 0, 3, 2, 5, 1, 6);
            if (unk32)
                packet.ReadSingle("unk32"); // 32
            if (unk36)
        ...
        [Parser(Opcode.SMSG_UNK_14D2)]
        public static void HandleUnk14D2(Packet packet)
        {
            var guid = new byte[8];
            var guid2 = new byte[8];
            ...
            packet.ParseBitStream(guid, 3);
            packet.ParseBitStream(guid2, 4);
            packet.ParseBitStream(guid, 4, 0);
            packet.ParseBitStream(guid2, 2);
            packet.ParseBitStream(guid, 7);
            packet.ParseBitStream(guid2, 3);
            packet.ParseBitStream(guid, 2);
